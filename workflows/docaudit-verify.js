...
Follow the docaudit verifier instructions. Read and cite only the repository (plus the mirror and index database in retrieval.indexCwd and retrieval.indexDb); a resource outside the repository is outside the audit scope, not missing. Search recursively before reporting a repository path as missing, including hidden directories such as .claude; if it exists elsewhere, the documented path is wrong, not missing. A directory inside the repository that has its own .git file or directory and is ignored by .gitignore (a worktree or nested checkout, for example under .worktrees/) is another checkout: a copy found only there is not evidence that this repository contains the file, does not count as the file existing elsewhere or as the location to report as the correct path, and does not contradict a document that says the file is no longer in the tree. Use only the presence of that .git file or directory; never read or cite its contents. Run every mdq command from the mirror directory in retrieval.indexCwd, never from the repository: mdq writes .mdq/usage.jsonl under its working directory, and any other file created inside the repository other than the assigned judgement makes the run REFUSED. Persist the judgement with a single Bash heredoc to "$PWD/${document.judgementPath}" (never resolve the path yourself, never use the Write tool), read it back with cat, and only then return the same structured object.`,
  {
    schema: JUDGEMENT,
